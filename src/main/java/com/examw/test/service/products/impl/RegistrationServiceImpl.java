...
public class RegistrationServiceImpl extends BaseDataServiceImpl<Registration,RegistrationInfo> implements IRegistrationService{
	...
	private Registration updateRegistration(RegistrationInfo info){
		...
		//创建注册码激活
		if((info.getStatus() == RegistrationStatus.ACTIVE.getValue()) && (data.getStatus() != RegistrationStatus.ACTIVE.getValue())){
			this.activeRegistration(data);
		}
		//属性赋值
	...
	public String autoGeneratedCode(String productId, String channelId,Integer limit) throws Exception {
		if(StringUtils.isEmpty(productId)) throw new Exception("产品ID为空!");
		Product product = this.productDao.load(Product.class, productId);
		if(product == null) throw new Exception("没有此产品的信息");
	...
	/*
	 * 激活注册码。
	 * @see com.examw.test.service.products.IRegistrationCodeService#activeRegistration(java.lang.String)
	 */
	@Override
	public boolean activeRegistration(String code) throws Exception {
		boolean result = this.verificationFormat(code);
		if(!result) throw new Exception("注册码错误！");
		Registration data = this.registrationDao.loadRegistration(code);
		if(data == null) throw new Exception(String.format("注册码未登记！"));
		return this.activeRegistration(data);
	}
	/*
	 * 激活注册码。
	 * @see com.examw.test.service.products.IRegistrationCodeService#activeRegistration(com.examw.test.domain.products.Registration)
	 */
	@Override
	public boolean activeRegistration(Registration data) {
		if(data == null) return false;
		//必须未激活
		if(data.getStatus() != RegistrationStatus.NONE.getValue()) return false;
		//激活
		int limit = this.limitTime;
